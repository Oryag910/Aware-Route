...
The two follow-ups above each tried to make ONE generator win at every supported count and neither closed both gates: polygon's full-suite p95 (2.241s+) stayed above the <2.0s gate, and its count=5 round reliability (93.3-93.9% all-within-tolerance) never closed v1's ~99.4% baseline. A third architectural attempt (an unmerged, closed experiment, `perf/polygon-topology-adaptive`) tried axis-scaled per-leg template refinement to close the count=5 gap directly; it nudged reliability from 93.3%->93.9% but made full-suite p95 worse (2.245s+), so it was closed without merging rather than trading one gap for a bigger one -- its branch and history remain on GitHub as a documented record, but none of its code is in this codebase.

This PR takes a different approach: since polygon is unambiguously better at the product's actual count=3 default and v1 is unambiguously better at count=5, pick the generator per-request based on which count was actually asked for, instead of forcing one generator to cover both. This requires first fixing a latent bug this migration would otherwise inherit: `facilities.orchestration.natural_match_pool` requests an intentionally OVERCOMPLETE candidate pool for its own downstream diversity/portfolio selection (e.g. 9 candidates for a real count=3 no-facility request, 12 for a count=3 request with facility requirements -- see [`architecture.md`](architecture.md#candidate-generation)), and prior to this PR that inflated pool size was the ONLY count `generate_routes` ever saw. Selecting a generator off it would have silently picked v1 for the product's actual count=3 default the moment facility requirements were present. `generate_routes` (and `_round_pairs`/`_round_generator_version` beneath it) now take an explicit, separate `requested_count` parameter that always carries the user's real `RouteRequest.count`, independent of whatever pool size is being constructed -- see `tests/generation/test_round_generator_seam.py` for direct proof this holds at every layer (unit-level `_round_generator_version` calls, `generate_routes` calls with pool sizes 3/9/12 all mapped to `requested_count=3`, and `natural_match_pool` itself, monkeypatched, asserting the exact pool-size vs `requested_count` split for both its no-facility and with-facility branches).

**Policy**: `ROUND_GENERATOR=auto` (the new default) selects polygon when `requested_count<=3`, v1 when `requested_count>=4`; `=v1`/`=polygon` force a single generator regardless of `requested_count`; unset or invalid still fails safe to `v1`... except "unset" is no longer possible to distinguish from `auto` now that `auto` IS the unset default -- an operator who explicitly wants the pre-PR-29 behavior back sets `ROUND_GENERATOR=v1`. Applies to explicit `shape="round"` and `shape="mix"`'s round component identically (same seam as before); `shape="out_and_back"` and the constrained multi-facility round planner (which already used polygon's machinery directly, independent of this flag) are both untouched.

**Fresh same-commit benchmarks of the ACTUAL `auto` policy** (not polygon-at-count=5 mislabeled as the product default, and not inferred from aggregate numbers alone -- `tests/generation/test_round_generator_seam.py` plus an ad-hoc instrumented run below directly assert which generator ran):
...
`auto`'s count=3 numbers are bit-for-bit identical (within measurement noise) to forced-`polygon`'s count=3 numbers, and `auto`'s count=5 numbers are bit-for-bit identical to forced-`v1`'s count=5 numbers -- direct evidence, not just an architectural argument, that `auto` is exercising the intended generator at each count. All gates pass: count=3 exact-count 100%/all-within 100%/p95<2.0s; count=5 all-within 99.4% (>=98.5% gate, matching v1's own baseline, nowhere near forced-polygon's 93.3%).

*Full 537-scenario suite* (`scripts/benchmark_suite.py`, which hardcodes `count=5` -- [report](../backend/benchmarks/local/report_20260828_223326.md)): under `auto` this correctly routes through v1 for that count, giving p95=1.505s (PASS <2.0s, matching v1's own ~1.5s baseline, not polygon's 2.24s+) and 537/537 within-tolerance. This is expected, not a loss of polygon's geometry for the real count=3 product path -- this suite has never exercised count=3.

*Geometry* (`scripts/benchmark_polygon_loop.py`, generator-level, count-independent -- [report](../backend/benchmarks/local/polygon_loop_v1_vs_v2_20260828_221249.md)): unchanged from prior migrations (isoperimetric quotient 0.160->0.465, elongation 5.13->1.41, radial exposure 0.404->0.299). Since `auto` selects polygon deterministically at `requested_count<=3` (proven above), this is `auto`'s own count=3 geometry by construction, not a separate inferred claim.

*Facility requests* (`scripts/benchmark_facilities.py` under `ROUND_GENERATOR=auto`, `count=3` throughout -- [report](../backend/benchmarks/facilities/report_20260828_222747.md)): Stratum A 117/117, Stratum B 18/18 -> 0/18 -> 0/18 -> 0/18 (unchanged known 2+ simultaneous-requirement boundary), Stratum C 12/12 -- bit-for-bit identical to the documented baseline. An additional instrumented check (spying on `polygon_loop_pairs`/`round_pairs` directly, not inferring from aggregate metrics) confirmed `auto` selects polygon for round/mix's round component at `requested_count=3` in every one of 6 representative scenarios (round and mix, x 0/1/2 facility requirements, where the internal pool inflates from 9 to 10-12 candidates as requirements are added) -- `polygon_calls>=1` and `v1_calls==0` in all 6, end-to-end latency 0.26-0.33s, confirming the pool-size/requested_count fix holds under real facility load, not just in isolated unit tests.

**Decision: `auto` is now the default.** Every acceptance gate passed on fresh same-commit evidence: count=3 (the product default) exact-count/within-tolerance both 100%, p95 well under 2.0s and matching polygon's own numbers; count=4/5 all-within-tolerance at v1's own ~99.4% baseline, comfortably above the 98.5% floor; full-suite and facility benchmarks show no regression. `ROUND_GENERATOR=v1` remains available as an immediate rollback (single env var, no code change) if a production issue surfaces; `ROUND_GENERATOR=polygon` remains available to force polygon at every count for further experimentation.
