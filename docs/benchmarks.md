...
**Fix 1**: `_tune_waypoints` now stops correcting a template as soon as two consecutive attempts build the identical distance (`PLATEAU_DISTANCE_EPSILON_M`), keeping whatever `best` attempt it already found. Pure latency win, provably zero effect on which candidate a plateaued template returns (the tracked `best` is unchanged by stopping early).
