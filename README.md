...
![Aware Route showing three ranked route alternatives with a matched restroom stop on the Upper East Side](docs/assets/aware-route-demo.png)
...
Needs `SUPABASE_URL` / `SUPABASE_KEY` for restroom data (only fetched when a request includes a restroom). Water data is a committed dataset — no extra config.
